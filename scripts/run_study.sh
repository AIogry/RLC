...
Usage:
  bash scripts/run_study.sh --study experiments/<study>/study.yaml \
    ...
    --train-steps N --batch-size N --eval-interval N \
    --eval-tasks N|all --eval-episodes N --save-interval N \
    --eval-temperature FLOAT [--eval-gaussian FLOAT] [--video-episodes N]

The formal launcher refuses a dirty Git worktree and requires common
training/evaluation protocol fields explicitly. It never prompts interactively.
EOF
}
...
VISIBLE_GPUS="$(nvidia-smi --query-gpu=index --format=csv,noheader | tr -d '[:space:]')" \
...
SUMMARY="$("$PYTHON_BIN" "$REPO_ROOT/tools/sweep.py" \
    --study "$STUDY_PATH" --gpus "$GPU_CSV" --run-root "$RUN_ROOT" \
    --summary-only)" || die 'Study validation/dry-run failed'
...
echo "Protocol: train_steps=$TRAIN_STEPS batch_size=$BATCH_SIZE eval_interval=$EVAL_INTERVAL eval_tasks=$EVAL_TASKS eval_episodes=$EVAL_EPISODES save_interval=$SAVE_INTERVAL eval_temperature=$EVAL_TEMPERATURE"
echo 'No interactive confirmation is required; the validated command is starting now.'
