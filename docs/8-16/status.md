...
## M9A SingleState iterative actor (2026-08-16)

- Audited official/RLC HIQL and CRL actor widths: all are `(512,512,512)`;
  audited real medium/large actor input/output shapes before implementation.
- Added decision-local `SingleState` topology with `K ∈ {1,2,4}`, shared MLP
  update, residual/non-residual equations, independent normal `buffers` RNG,
  non-trainable checkpointed `z_init`, and direct full-BPTT credit.
- Added generic `TrainState.model_state` support while preserving empty baseline
  state and old checkpoint restore compatibility.
- Kept official HIQL high and low actors as independent GCActor networks; the
  high+low placement has separate parameters and buffers, with no shared core.
- Added canonical M9A Study overrides, 26 configurations, 52 planned rows,
  parameter/buffer accounting, and a safe GPU worker with explicit execution.
- SingleState unit and study-placement tests pass; a real 1-step CPU CRL K=1
  smoke passed with finite loss, evaluation, checkpoint save/restore, and
  `actor_parameter_accounting` metadata. Full CPU regression is `65/65`; five
  1-step GPU placement smokes passed with `jax_backend=gpu`. Details are in
  `docs/m9_single_state_iterative.md`.

M9A implementation is integration validated for the short vertical slice only;
no full scientific sweep or performance conclusion has been made.

## M9B TwoState hierarchical actor (2026-08-16)

- Audited the local M9A source and HRM-mini reference; actor hidden dimensions
  remain `(512,512,512)` and HRM-only Transformer/SwiGLU/RMSNorm/carry semantics
  were excluded.
- Added decision-local `TwoState` with independent H/L GELU MLP updates,
  H2L1/H2L6 schedules, standard-normal H/L buffers, and final representation
  `z_H`.
- Added executable `full_bptt` and `one_step` credit. One-step truncates only
  before the final L→H pair; production/manual forward outputs match while
  gradients differ as intended.
- Added M9B Study: 16 configurations, 32 planned runs, explicit M9A baseline
  references, extended accounting/manifest fields, and reused generic run/sweep
  tooling.
- TwoState/M9B targeted tests pass `11/11`; full CPU regression passes `76/76`.
  Five 1-step real-data GPU smokes passed with `jax_backend=gpu`, evaluation,
  checkpoint restore, and accounting metadata.

M9B is integration validated for the short vertical slice only. No formal
scientific sweep or performance conclusion has been made.
