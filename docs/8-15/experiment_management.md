...
`experiments/M9A_single_state_iteration/` is a planned, non-executable Study.
Its primary factors are internal iterations `K` and residual versus
non-residual update. The fixed design is decision-local, non-learned state,
`z+x` injection, an MLP update module, shared parameters, and no state across
environment steps. State initialization alternatives, other injection forms,
gating, normalization recipes, multi-state, and HRM are deferred.

No SingleState topology is created and no M9A training is started.
