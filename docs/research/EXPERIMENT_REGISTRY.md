...
| M23A-F0 | Observation-only Puzzle physical press/rollout diagnostic foundation | foundation merged; no formal Study/results | source under `impls/diagnostics/puzzle/` and `tools/run_puzzle_event_diagnosis.py` | [`M23A-F0 context`](experiments/M23A-F0/README.md) |
